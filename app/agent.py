...
# Radice del progetto (questo file vive in app/): usata per ancorare i percorsi
# di tmp/ (ChromaDB, SQLite) alla posizione del file invece che alla working
# directory di invocazione — un "tmp/chromadb" relativo si romperebbe (creando
# silenziosamente un DB vuoto altrove) se qualcuno lanciasse lo script da una
# cwd diversa dalla radice, es. `cd app && python main.py`.
...
_vector_db = ChromaDb(
    ...
    path=str(_PROJECT_ROOT / "tmp" / "chromadb"),
...
db = SqliteDb(db_file=str(_PROJECT_ROOT / "tmp" / "cinema_traces.db"))
